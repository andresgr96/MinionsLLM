...
class BehaviorTreeGrammarValidator:
    ...
    def __init__(self, grammar_rules: Dict[str, Any]):
        ...
        self.parsed_rules = self._parse_grammar_to_validation_rules(
            self.raw_grammar_rules
        )
        self.flattened_grammar_shapes = self._flatten_grammar_shapes(self.parsed_rules)

    def _parse_grammar_to_validation_rules(
        ...
        for symbol, rule in grammar_rules.items():
            if (
                symbol.islower()
                and isinstance(rule, list)
                and len(rule) == 2
                and isinstance(rule[1], list)
                and rule[1] == ["children_nodes"]
            ):
        ...
        for terminal_symbol, node_type in terminal_nodes.items():
            ...
            for rule_children in non_terminal_rules:
                combination = self._process_rule_children(
                    rule_children, grammar_rules, amounts
                )
                if combination:
        ...
        for symbol in children_symbols:
            if symbol.endswith("n"):
                if symbol in grammar_rules and len(grammar_rules[symbol]) > 1:
                    can_be_empty = any(
                        rule == [] for rule in grammar_rules.get(symbol, [])
                    )
                    min_amount = 0 if can_be_empty else 1
                    single_rep = grammar_rules[symbol][1]
                    options = self._expand_symbol_fully_from_rule(
                        single_rep, grammar_rules
                    )
    ...
    def _expand_symbol_fully_from_rule(
        self, rule: Any, grammar_rules: Dict[str, Any]
    ) -> List[List[str]]:
            ...
            if isinstance(symbol, str):
                if symbol in grammar_rules:
                    if symbol.islower():
                        ...
                        expanded_rule.append(
                            terminal_rule[0] if len(terminal_rule) >= 1 else symbol
                        )
                    else:
    ...
    def _expand_symbol_fully(
        ...
        if symbol.islower():
            ...
            if len(terminal_rule) == 2 and terminal_rule[1] == ["children_nodes"]:
                return [[terminal_rule[0]]]
                    ...
                    if isinstance(sub_symbol, str):
                        if sub_symbol in grammar_rules:
                            if sub_symbol.islower():
                                ...
                                expanded_rule.append(
                                    self._expand_symbol_fully(
                                        sub_symbol, grammar_rules, visited.copy()
                                    )
                                )
                        else:
                            expanded_rule.append(sub_symbol)
    ...
    def _mark_unlimited_amounts(
        self, options: List[List[str]], amounts: Dict[str, int], min_amount: int
    ) -> None:
    ...
    def _flatten_grammar_shapes(
        ...
        for node_type, rules in parsed_rules.items():
            ...
            for combination in rules["Combinations"]:
                shapes_from_combo = self._generate_flattened_shapes_from_combination(
                    combination
                )
    ...
    def _generate_flattened_shapes_from_combination(
        self, combination: List[Any]
    ) -> List[List[str]]:
    ...
    def _extract_tree_shapes(self, xml_string: str) -> Dict[str, Any]:
        try:
            ...
            def remove_consecutive_duplicates(
                child_types: List[str], parent_node_type: str
            ) -> List[str]:
                if not child_types:
                    return []
                unlimited_nodes = set(
                    self.parsed_rules.get(parent_node_type, {})
                    .get("Amounts", {})
                    .keys()
                )
                ...
                for i in range(1, len(child_types)):
                    if (
                        child_types[i] != child_types[i - 1]
                        or child_types[i] not in unlimited_nodes
                    ):
            ...
            def extract_node_shape(node: ET.Element) -> None:
                ...
                if children:
                    ...
                    collapsed_child_types = remove_consecutive_duplicates(
    ...
    def validate_tree(self, tree_xml: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            root = ET.fromstring(tree_xml.strip())
        except ET.ParseError as e:
            # This catches completely malformed XML that can't be parsed at all.
            return False, f"XML parsing error: {e}"

        # A valid BehaviorTree must have at least one child element.
        # An empty <BehaviorTree></BehaviorTree> or <BehaviorTree>text</BehaviorTree> is invalid.
        if len(list(root)) == 0:
            return (
                False,
                "The <BehaviorTree> root node must have at least one child node to be syntactically valid.",
            )
        ...
        if errors:
            return False, f"Tree has syntax errors: {'; '.join(errors)}"
