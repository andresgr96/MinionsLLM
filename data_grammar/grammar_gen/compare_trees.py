...
def validate_node_recursive(orig_node: ET.Element, gen_node: ET.Element, 
    ...
    if orig_node.tag != gen_node.tag:
        feedback.append(f"Node tag mismatch at {current_path}: expected '{orig_node.tag}' but got '{gen_node.tag}'")
        return False
    ...
    if len(orig_node) != len(gen_node):
        feedback.append(f"Number of children mismatch at {current_path}: expected {len(orig_node)} but got {len(gen_node)}")
        return False
    
    # Recursively validate all children
    return all(validate_node_recursive(orig_child, gen_child, conditions, actuator_actions, state_actions, feedback, current_path) 
...
def validate_tree_structure(original_tree: str, generated_tree: str, 
    ...
    try:
        ...
        try:
            ...
            return False, f"Generated tree parsing error at line {e.position[0]}, column {e.position[1]}: {str(e)}"
