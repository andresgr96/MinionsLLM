...
def process_tree_with_api(tree_content: str, 
    ...
    for attempt in range(max_retries + 1):  # +1 for initial attempt
        ...
        if completion.choices[0].message.parsed:
            ...
            print(f"Attempt {attempt + 1}/{max_retries + 1} failed: Could not parse structured response")
        ...
        if file_name.endswith(".xml") and (max_trees is None or processed_count < max_trees):
            ...
            try:
                ...
                if task and tree:  # Only add to dataset if generation was successful
                    ...
                    processed_count += 1
                    max_trees_display = max_trees if max_trees is not None else "unlimited"
                    print(f"------------Produced: {processed_count} of {max_trees_display} trees------------")
                else:
                    print(f"Skipping {file_name} due to validation or metrics failures\n")
            except Exception as e:
