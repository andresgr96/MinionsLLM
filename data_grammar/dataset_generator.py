...
from data_grammar.grammar_gen.node_translations import node_connectors as default_connectors
...
class DatasetGenerator:
    ...
    def _generate_trees(self, 
        ...
        if not placeholders and filter_env is not None:
            return self._generate_filtered_trees(n_trees, size, filter_env, output_dir, filter_metrics, params_to_use, start_index)
        
        # Original logic for non-filtered generation
        print(f"Generating {n_trees} trees with size {size} and placeholders: {placeholders}")
    ...
    def _generate_filtered_trees(self, 
                               target_trees: int, 
                               size: int, 
        ...
        if filter_metrics is None:
            raise ValueError("filter_metrics must be specified when using filtering. No default metrics are provided.")
        ...
        while len(tree_paths) < target_trees:
            ...
            if attempts % 100 == 0:
                print(f"Attempt {attempts}: Found {len(tree_paths)}/{target_trees} valid trees")
            ...
            if self._test_tree_metrics(xml_tree, filter_env, filter_metrics):
                # Save valid tree
                tree_path = output_dir / f"behavior_tree_{start_index + len(tree_paths)}.xml"
                str_tree = pretty_print_xml(xml_tree)
        ...
        print(f"Filtering complete: {len(tree_paths)} valid trees found in {attempts} attempts")
        return tree_paths
    ...
    def _generate_mixed_structure_trees(self,
        ...
        for i, structure_config in enumerate(self.grammar_parameters_list):
            ...
            if use_separate_folders:
                ...
                print(f"Structure {i+1}/{len(self.grammar_parameters_list)}: Generating {trees_to_generate} trees in {structure_dir}")
                ...
                print(f"Structure {i+1}/{len(self.grammar_parameters_list)}: Generating {trees_to_generate} trees starting from index {start_index}")

            # Generate trees with this specific structure
            # Use structure-specific metrics if available and filtering is enabled
            metrics_to_use = structure_metrics if (structure_metrics is not None and filter_env is not None) else filter_metrics
            ...
            print(f"Structure {i+1} base generation complete: {len(structure_trees)} trees created")
        
        if use_separate_folders:
            print(f"Mixed structure generation complete: {len(structure_info)} structure folders created")
    ...
    def _test_tree_metrics(self, xml_tree: Any, filter_env: SimEnvironment, filter_metrics: Union[List[str], Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            if self._check_metrics_against_targets(metrics, filter_metrics):
                print(f"Tree {pretty_print_xml(xml_tree)}\n passed with metrics: {metrics}")
                return True
                ...
                if isinstance(target_value, list) and isinstance(actual_value, list):
                    ...
                    if target_value == 0:
                        ...
                        if actual_value < target_value:
                            return False
                elif isinstance(target_value, (int, float)) and isinstance(actual_value, list):
    ...
    def generate_dataset_a(self, 
        ...
        """
        from data_grammar.dataset_generation.api_gen_dataset_a import process_trees_in_folder
        ...
        print(f"Generating populated trees for dataset {dataset_name} and with filtering: {to_filter}...")

        if self.mixed_structures:
            # Calculate max_trees_to_process based on trees_per_params (first element of each structure config)
            max_trees_to_process = sum(config[0] for config in self.grammar_parameters_list)
        ...
        if self.mixed_structures and isinstance(structure_result, list) and len(structure_result) > 0 and isinstance(structure_result[0], tuple):
            ...
            for i, (structure_dir, target_count, structure_metrics) in enumerate(structure_result):  
                ...
                if temp_output.exists():
                    ...
                    print(f"Structure {i+1} contributed {len(structure_entries)} entries")
            ...
            print(f"Combined dataset saved with {len(all_dataset_entries)} total entries")
    ...
    def generate_dataset_b(self, 
        ...
        Returns:
            Path to the generated dataset
        """
        from data_grammar.dataset_generation.api_gen_dataset_b import process_trees_in_folder
