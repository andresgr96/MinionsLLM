...
class BehaviorTreeGenerator:
    def __init__(self,
        ...
        if backend not in ["llamacpp", "ollama"]:
            raise ValueError(f"Invalid backend '{backend}'. Must be 'llamacpp' or 'ollama'")
    ...
    def _validate_ollama_requirements(self, model_path_or_url: str, ollama_model_name: str) -> None:
        ...
        if not model_path_or_url and not ollama_model_name:
            raise ValueError("Either model_path_or_url or ollama_model_name is required for ollama backend")
    ...
    def _init_ollama(self, model_path_or_url: str, ollama_model_name: str, **kwargs) -> None:
        ...
        if ollama_model_name:
            ...
            elif 'hf.co/' in model_path_or_url or 'huggingface.co/' in model_path_or_url:
                # Pull from HuggingFace
                print(f"Pulling model from HuggingFace: {model_path_or_url}")
                self.ollama_model = self._pull_model(model_path_or_url)
            else:
                raise ValueError("model_path_or_url must be either a .gguf file path or a HuggingFace URL")
    ...
    def _pull_model(self, model_name: str) -> str:
        ...
        try:
            ...
            if "401" in error_str and "Invalid username or password" in error_str:
                ...
                print("="*60)
                print("You are trying to access a gated/private model from HuggingFace. First make sure you have access to the model.")
                print("To access gated models with Ollama, you need to set up SSH key authentication.")
                print("\nFollow these steps:")
                print("\n1. Get your Ollama SSH key:")
                print("   Windows: Get-Content $env:USERPROFILE\\.ollama\\id_ed25519.pub")
                ...
                print("https://huggingface.co/docs/hub/ollama#run-private-ggufs-from-the-hugging-face-hub")
                print("="*60)
                
                raise RuntimeError("SSH key authentication required for gated model. Please follow the instructions above.")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise RuntimeError(f"Failed to import GGUF file. CLI error: ollama command not found. API error: {create_error}. Stream error: {stream_error}")
        finally:
    ...
    def _verify_ollama_model(self, model_name: str) -> None:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Cannot connect to Ollama service. Make sure Ollama is running. Error: {e}")
            ...
            if search_model_name not in available_models:
                ...
                if not model_name.startswith('custom-'):
                    print(f"Model '{search_model_name}' not found in Ollama. Attempting to download...")
                    self._pull_model(search_model_name)
                else:
                    raise ValueError(f"Custom model '{search_model_name}' not found. It may need to be reimported.")
            else:
    ...
    def query_vector_db(self, query: str) -> List:
        ...
        try:
            ...
            results = self.vector_tb.search(embedded_query).limit(2).to_pydantic(self.schema)
    ...
    def generate_behavior_tree(self, prompt: str, fix_mistake: str = "", which_prompt: int = 2, log_prompt: bool = False) -> tuple[int, str]:
        ...
        if which_prompt == 5 and not self.rag_enabled:
            raise RuntimeError("RAG prompt selected but RAG is not enabled. Please provide client, vector_tb, and schema.")
    ...
    def extract_behavior_tree(self, response: str) -> str:
        ...
        print(f"Current error count: {self.error_count}, Max retries: {self.max_retries}")  # Debug print
        
        if start_index == -1 or end_index == -1:
            if self.error_count >= self.max_retries:
                print(f"Hit max retries ({self.max_retries}), returning error message")  # Debug print
    ...
    def _suggest_similar_models(self, model_name: str) -> None:
        ...
        print(f"Or list all available models with: ollama list")
