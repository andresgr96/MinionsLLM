...
class AgentDocstringParser:
    ...
    def _should_include_method(self, method_name: str) -> bool:
        ...
        if (callable(getattr(self.agent_class, method_name)) and 
            not method_name.startswith("__") and
            not method_name.startswith("update") and
            not method_name.startswith("obstacle") and
            not method_name.startswith("helper")):
    ...
    def _parse_method_docstring(self, method_name: str, docstring: str) -> None:
        ...
        if not translation:
            warnings.warn(f"'Translation' not found in docstring for method '{method_name}'. If using this for dataset generation, you will need to provide a translation for this method, otherwise ignore this warning.")
        if not spoon_translation:
            warnings.warn(f"'Spoon Translation' not found in docstring for method '{method_name}'. If using this for dataset generation, you will need to provide a spoon translation for this method, otherwise ignore this warning.")
