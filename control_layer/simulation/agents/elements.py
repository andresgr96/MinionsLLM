...
class Part(Agent):
    ...
    def drop_at_location(self, in_base, in_waste, in_storage, in_construction, in_source):
        ...
        if in_base:
            ...
            self.owner = self.env
            self.pos = Vector2(self.env.base_pos.x + noise_x, self.env.base_pos.y + noise_y)

            if in_storage:
                self.env.parts_dropped_in_storage[part_idx] += 1
                self.pos = Vector2(self.env.storage_pos.x + noise_x, self.env.storage_pos.y + noise_y)

            elif in_construction:
                self.env.parts_dropped_in_construction[part_idx] += 1
                self.pos = Vector2(self.env.construction_pos.x + noise_x, self.env.construction_pos.y + noise_y)

        elif in_waste:
            self.env.parts_dropped_in_waste[part_idx] += 1
            self.is_permanently_placed = True
            self.pos = Vector2(self.env.waste_pos.x + noise_x, self.env.waste_pos.y + noise_y)
            self.owner = self.env

        elif in_source:
            self.env.parts_dropped_in_source[part_idx] += 1
            self.is_permanently_placed = True
            self.pos = Vector2(self.env.source_pos.x + noise_x, self.env.source_pos.y + noise_y)
            self.owner = self.env
        else:
