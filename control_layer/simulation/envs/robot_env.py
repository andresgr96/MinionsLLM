...
class RobotEnvironment(SimEnvironment):
    def __init__(self, config: Config, bt_path: str, n_agents: int, n_parts: int, task: str, headless: bool):
        ...
        # Preloaded Images
        self.loaded_agent_images = self.load_images(["./control_layer/simulation/images/white.png", "./control_layer/simulation/images/red circle.png", "./control_layer/simulation/images/green.png"])
        self.loaded_parts_imgs = self.load_images(["./control_layer/simulation/images/part_green.png", "./control_layer/simulation/images/part_red.png"])

    def draw_obstacle(self) -> None:
        x = 350
        y = 100
        self.simulation.spawn_obstacle("./control_layer/simulation/images/rect_obst.png", x, y)

    def draw_arena(self) -> None:
        self.simulation.spawn_obstacle("./control_layer/simulation/images/arena_new.png", self.arena_pos.x, self.arena_pos.y)

    def draw_source(self) -> None:
        self.simulation.spawn_site("./control_layer/simulation/images/source_green.png", self.source_pos.x, self.source_pos.y)

    def draw_nest(self) -> None:
        self.simulation.spawn_site("./control_layer/simulation/images/blue_nest.png", self.base_pos.x, self.base_pos.y)

    def draw_waste(self) -> None:
        self.simulation.spawn_site("./control_layer/simulation/images/waste_red.png", self.waste_pos.x, self.waste_pos.y)
    ...
    def place_parts(self, num_parts: int) -> None:
        for _ in range(num_parts):
            ...
            # Place the bad parts
            rand_bad_pos = Vector2(random.uniform(self.arena_pos.x - 100, self.arena_pos.x + 150), random.uniform(self.arena_pos.y - 50, self.arena_pos.y + 50))
    ...
    def setup(self) -> None:
        ...
        for _ in range(self.n_agents):
            ...
            agent = RobotAgent(images=self.loaded_agent_images, simulation=self.simulation,
                                pos=Vector2(self.base_pos.x + noise_x, self.base_pos.y + noise_y), env=self, xml_path=self.xml_path)
    ...
    def run(self) -> Dict[str, Any]:
        ...
        nest_integrity = 100 if nest_integrity > 100 else 0 if nest_integrity < 0 else nest_integrity
