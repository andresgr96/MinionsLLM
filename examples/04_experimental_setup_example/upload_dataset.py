...
def upload_to_hub(dataset: Dataset, repo_id: str):
    """Upload the dataset to the Hugging Face Hub. Formats dataset in gemma-3 format."""
    dataset.push_to_hub(
        repo_id,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to unformatted dataset JSON. Needs to follow the format generated by DatasetGenerator"
    )
    parser.add_argument(
        '--repo_id',
        type=str,
        required=True,
        help="HuggingFace Hub repository ID (e.g., 'username/dataset-name'). If not provided, the dataset will not be uploaded."
    )
    parser.add_argument(
        '--token',
        type=str,
